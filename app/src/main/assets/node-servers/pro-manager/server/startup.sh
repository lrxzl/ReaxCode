...
if [ -f "$PID_FILE" ]; then
    ...
    if [ -n "$OLD_PID" ] && kill -0 "$OLD_PID" 2>/dev/null; then
        echo "[pro-manager] 已在运行 (PID: $OLD_PID)，跳过"
        exit 0
    fi
fi
rm -f "$PID_FILE"
...
for i in $(seq 1 60); do
    ...
    sleep 1
    [ $i -eq 60 ] && { echo "[pro-manager][错误] node 未就绪"; exit 1; }
done

# ---------- 3. 安装依赖 ----------
install_deps() {
    echo "[pro-manager] 开始安装依赖..."

    if [ -f "package.json" ]; then
        npm install --no-audit --no-fund 2>&1 || {
            echo "[pro-manager][错误] 根目录依赖安装失败"
            return 1
        }
    fi
    ...
    if [ -d "server" ] && [ -f "server/package.json" ]; then
        (cd server && npm install --no-audit --no-fund 2>&1) || {
            echo "[pro-manager][错误] server 依赖安装失败"
            return 1
        }
    fi
    ...
    if [ -d "server/html" ]; then
        echo "[pro-manager] 检测到 server/html/，跳过前端构建"
    elif [ -d "web" ] && [ -f "web/package.json" ]; then
        echo "[pro-manager] 构建 web 前端..."
        (
            cd web
        ...
        ) && echo "[pro-manager] 前端构建成功" \
          || echo "[pro-manager] ⚠️ 前端构建失败，下次启动会重试"
    fi
...
if ! verify_deps; then
    install_deps || {
        echo "[pro-manager][错误] 依赖安装失败，放弃启动"
        exit 1
    }
else
    echo "[pro-manager] 依赖已就绪，跳过安装"
fi
...
if [ -f "index.js" ]; then
    ...
    echo "[pro-manager][错误] 找不到入口文件"
    exit 1
fi

# ---------- 5. 启动服务 ----------
echo "[pro-manager] 启动 $ENTRY..."
...
if kill -0 "$NEW_PID" 2>/dev/null; then
    echo "[pro-manager] 启动成功 (PID: $NEW_PID)"
    exit 0
else
    echo "[pro-manager][错误] 进程已退出，请查看 $LOG_FILE"
    rm -f "$PID_FILE"
    exit 1
fi
